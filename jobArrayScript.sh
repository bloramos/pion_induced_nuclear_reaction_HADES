...
#   EDIT THIS PART TO EXECUTE YOUR JOB!
   ...
   par4=${par4}/${filename}_$SLURM_ARRAY_TASK_ID.root
...
#----------------------
  ...
  echo "==> $par2 $par3 $par4 $par5 $par6 $par7"
  time  $par2 $par3 $par4 $par5 $par6 $par7
  ...
  if [ $status -ne 0 ]
    then
    echo "JOB: $SLURM_JOBID CRASHED ON HOST: $host WITH OUTFILE $outfile_wo_path"
    echo "JOB: $SLURM_JOBID CRASHED ON HOST: $host WITH OUTFILE $outfile_wo_path" > ${outdir}/crash/${outfile_wo_path}_crash.txt
  fi
